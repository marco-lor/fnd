...
const PlayerInfoLingueRow = ({ users, iconDeleteClass, onDeleteLingua, variant = "table" }) => {
  const renderList = (user) => {
    if (!user.lingue || !Object.keys(user.lingue).length) {
      return <span className="text-gray-400 italic text-sm">No lingue</span>;
    }
    return (
  ...
  if (variant === "card") {
    return (
      <div className="space-y-2">
        <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Lingue</div>
        {users.map((user) => (
          <div key={`${user.id}-li-card`} className="rounded-lg border border-slate-700/50 bg-slate-800/70 p-3">
            {renderList(user)}
          </div>
        ))}
      </div>
