...
const PlayerInfoDiceRollsRow = ({ users }) => {
      ...
      {users.map(user => {
        ...
        return (
          <td key={`${user.id}-dice`} className="border border-gray-600 px-4 py-2 text-xs">
            {rolls.length ? (
              <>
              {/* Show up to 5 items worth of height (~5 * 34px ≈ 170px). Remaining (up to 20 fetched) accessible via scroll. */}
              ...
              <span className="text-gray-500 italic">No rolls</span>
            )}
          </td>
        );
      })}
