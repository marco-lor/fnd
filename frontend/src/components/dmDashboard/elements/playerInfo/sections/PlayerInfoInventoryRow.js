...
const PlayerInfoInventoryRow = ({
  ...
  const renderCellContent = (user) => {
    ...
    const list = [...numbered, ...Object.values(varieMap).sort((a, b) => a.name.localeCompare(b.name))];
    ...
    return (
      <div className="align-top">
        <div className="mb-3">
          <div className="inline-flex flex-wrap items-center gap-2 rounded-full border border-amber-500/60 bg-slate-900/60 px-3 py-1">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                className="flex h-7 w-7 items-center justify-center rounded-full border border-red-500/70 text-red-300 transition hover:bg-red-500/20 disabled:opacity-50"
                onClick={() => onOpenGoldOverlay(user.id, -1)}
                disabled={goldBusy}
                title="Rimuovi gold"
              >
                <FontAwesomeIcon icon="minus" className="w-3 h-3" />
              </button>
            </div>
            <div className="flex items-center gap-2 pl-2">
              <button
                className="flex h-8 items-center gap-2 rounded-md border border-slate-600 bg-slate-800 px-2 text-xs font-medium text-slate-100 transition hover:bg-slate-700 disabled:opacity-50"
                onClick={() => setGrantUserId(user.id)}
                disabled={!canGrantBazaar}
                title="Concedi un oggetto dal bazaar"
              >
                <span>+ Bazaar</span>
              </button>
              <button
                ...
                title="Aggiungi un oggetto Varie"
              >
                <span>+ Varie</span>
              </button>
            </div>
  ...
  const cardMarkup = (
    <div className="space-y-2">
      <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Inventario</div>
      {users.map((user) => (
        <div key={`${user.id}-inv-card`} className="rounded-lg border border-slate-700/50 bg-slate-800/70 p-3">
          {renderCellContent(user)}
        </div>
      ))}
    </div>
  ...
  return (
    <>
      {variant === "card" ? cardMarkup : tableMarkup}
      {deleteTarget && (
