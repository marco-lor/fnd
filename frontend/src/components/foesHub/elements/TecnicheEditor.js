...
const TecnicheEditor = ({ value = [], onChange }) => {
  ...
  return (
    <div className="space-y-3">
      <button type="button" onClick={addItem} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600/80 hover:bg-emerald-600 text-white px-3 py-1 border border-emerald-400/40 text-sm">+ Add Tecnica</button>
      {(list || []).length === 0 ? (
        <div className="text-slate-400 text-sm">No tecniche added yet.</div>
      ) : (
        <div className="space-y-3">
          {list.map((it, idx) => (
              ...
              <div className="flex items-start gap-3">
                <div className="w-20 h-20 rounded-lg overflow-hidden border border-slate-700/60 bg-slate-800/60 flex items-center justify-center shrink-0">
                  {(it.previewUrl || it.imageUrl) ? (
                    <img src={it.previewUrl || it.imageUrl} alt={it.name || 'tecnica'} className="w-full h-full object-cover" />
                  ) : (
