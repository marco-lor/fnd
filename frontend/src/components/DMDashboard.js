...
// Import the lock icons from your preferred icon library, here using Font Awesome as an example
...
const DMDashboard = () => {
  ...
  useEffect(() => {
    // Check if user data is loaded and has DM role
    if (!userData) {
      return; // Still loading user data
    }

    if (userData.role !== "dm") {
      // Not a DM, redirect to home
      console.log("Access denied: User is not a DM");
      navigate("/home");
      return;
    }

    // User is confirmed as DM, fetch all users (just once, not with live updates)
