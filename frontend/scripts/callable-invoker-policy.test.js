...
test('execution requires exact project confirmation and plan fingerprint', () => {
  ...
  assert.throws(
    ...
    /execution is hard-locked to fatin-test\/europe-west8/i
  );
  assert.throws(
    () => parseArguments([
      '--project', PROJECT_ID,
...
test('execution updates only planned drift, preserves unrelated IAM, and fully re-verifies', async () => {
  const unrelated = {
    ...
    members: ['serviceAccount:audit@fatin-test.iam.gserviceaccount.com'],
  };
  const backend = createBackend({
    policies: allPolicies({
      task05AdjustGold: privatePolicy([unrelated]),
...
test('Firebase Functions postdeploy runs every strict read-only IAM check', () => {
  ...
  assert.deepEqual(functionsConfig.postdeploy, [
    'node "$PROJECT_DIR/scripts/callable-invoker-policy.js" --project fatin-test --region europe-west8 --check',
    'node "$PROJECT_DIR/scripts/callable-invoker-policy.js" --project fatin-test --region europe-west1 --check',
    'node "$PROJECT_DIR/scripts/callable-invoker-policy.js" --project fatin-test --region us-central1 --check',
  ]);
});
