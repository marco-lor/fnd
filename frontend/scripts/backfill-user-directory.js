...
const printHelp = () => {
  console.log([
    ...
    '  - Live access is hard-locked to fatin-test, Firebase CLI auth, and exact confirmation.',
    '  - Production fatins is always refused.',
  ].join('\n'));
};
...
const assertSafeTarget = (options, env = process.env) => {
  ...
  if (projectId !== LIVE_TEST_PROJECT_ID) {
    throw new Error(`This isolated backfill accepts only live project ${LIVE_TEST_PROJECT_ID}.`);
  }
  if (!options.allowLiveProject || options.confirmProject !== projectId) {
    throw new Error(
      'Live Firestore access requires --allow-live-project and exact --confirm-project fatin-test.'
    );
  }
  if (options.authMode !== 'firebase-cli') {
    throw new Error('Live fatin-test access requires --auth firebase-cli.');
  }
