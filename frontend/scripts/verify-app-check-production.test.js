...
const createFetch = ({
  ...
  if (
    url.includes('firebaseappcheck.googleapis.com/v1/projects/fatin-test/apps/')
  ...
  if (url.includes('recaptchaenterprise.googleapis.com/v1/projects/fatin-test/keys/')) {
...
test('production target requires exact project, confirmation, and Firebase CLI auth', () => {
  ...
  assert.throws(
    () => assertSafeTarget({...options, projectId: 'wrong-project'}, {}),
    /only live project fatin-test/
  );
  assert.throws(
  ...
  assert.throws(
    () => assertSafeTarget(options, {GCLOUD_PROJECT: 'wrong-project'}),
    /does not match/
  );
});
