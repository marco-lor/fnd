...
Use `npm run perf:authoritative` on the named reference machine to perform two compatible runs. Each run produces one discarded warmup and three retained measurements per single-context scenario. `npm run perf:repeatability` requires deterministic metrics and build assets to match and timing medians to remain within 15%; the per-iteration `runtime.maxLongTaskMs` series instead compares each run's retained maximum so its worst-case name, repeatability report, aggregate report, and baseline value stay aligned. Measurement contract v5 retains separate raw repeatability, evidence-completeness, timing-variance, and execution-gate statuses. A controlled reference machine remains strict: any failed timing comparison blocks. On the exact `github-hosted-runner` identity, complete finite iteration-matched timing variance is retained as failed advisory evidence while the execution gate may continue to the blocking budget comparison. Missing samples, iteration mismatch, compatibility failure, deterministic drift, browser failure, and unknown or mismatched machine identities still block. The contract also treats a local-emulator TTFB difference of at most 5 ms as timer jitter while retaining raw variance; on the hosted runner only, it records but does not gate differences within the Long Tasks API's 50 ms entry threshold/jitter band or within 1 ms for sub-millisecond microbenchmarks. The aggregate contains all six retained measurements. GitHub's scheduled authoritative timing remains informational because a hosted runner is not a baseline source.

Use `npm run perf:baseline -- --accept` only after reviewing a passing `performance-results/repeatability-report.json` produced on a named controlled reference machine. Baseline acceptance requires matching raw, evidence, timing, and execution-gate PASS statuses in strict mode under the contract's exact 15% threshold. It verifies the aggregate checksum and matching embedded repeatability fields so stale, advisory, custom-threshold, or hand-combined results cannot be accepted, and it rejects GitHub-hosted, unknown, or missing reference-machine identities.
