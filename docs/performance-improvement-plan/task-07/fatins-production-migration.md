...
1. Work on the reviewed production branch (`devs` unless the release process
   explicitly changes it) with a clean tree. Record the starting commit.
...
3. Obtain an independent code and deployment review. Deploy the reviewed
   Firestore rules/indexes, Storage rules, Functions, and Hosting release while
   control remains `v1-write`. Confirm the exact production Function regions,
   bucket CORS, IAM invokers, App Check, and served bundle.
...
1. Run the temporary-object hygiene discovery. Zero is accepted directly. A
   nonzero result requires a count-bound plan, item-by-item independent review,
   exact fingerprint approval, generation-fenced deletion, and a new baseline
   inventory. Eligible `.tmp-` objects must either have a verified canonical
   twin or have a `deleted` manifest, absent final, and exactly `complete`
   cleanup ledger; all other missing-final cases block.
2. Create the all-15-source backfill discovery from production. Derive the
   production candidate count; do not assume the seven repair candidates found
   in `fatin-test` exist in `fatins`.
...
4. Run the production canonical audit twice: discovery, then exact-count
...
5. Create a fresh production control dry-run from the approved production
...
2. Immediately rerun the bound canonical audit and the preservation inventory.
   Require zero missing or changed pre-existing objects.
...
4. Wait for the disposable records' normal retention/cleanup state, then run a
   final canonical audit, a final full-bucket preservation comparison, and a
   temporary-object cleanup dry-run bound to expected count zero.
